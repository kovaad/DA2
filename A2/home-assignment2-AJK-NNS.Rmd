...
```
First, we collected the descriptive statistics for our London sample. The table above indicates that 43% of the hotels in London are highly rated. Regarding the other two variables, the distribution of distance from center is right-skewed with an average of 2.60 kilometers, while we can infer a roughly normal distribution (mean ~ median) for stars with an average of 3.45.
...
```{r, echo=FALSE, fig.align='center', warning = FALSE, fig.height= 4, fig.width=4}
